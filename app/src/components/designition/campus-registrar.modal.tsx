...
const campusRegistrarSchema = z.object({
  user_id: z.number().min(1, 'User is required'),
  designitionable_id: z.number().min(1, 'Campus is required'),
  is_active: z.boolean(),
});

type CampusRegistrarFormData = z.infer<typeof campusRegistrarSchema>;

interface CampusRegistrarModalProps {
  controller: ModalState<Designition>;
  onSubmit: (data: CampusRegistrarFormData) => void;
}
...
function CampusRegistrarModalContent({ controller, onSubmit }: CampusRegistrarModalProps) {
  const {
    handleSubmit,
    reset,
    setError,
    ...
    setValue,
    formState: { errors },
  } = useForm<CampusRegistrarFormData>({
    resolver: zodResolver(campusRegistrarSchema),
    defaultValues: {
      user_id: 0,
  ...
  const { mutateAsync: createCampusRegistrar, isPending } = useCreateCampusRegistrar();
  ...
  useEffect(() => {
    if (campuses.length > 0 && watch('designitionable_id') === 0 && !isEdit) {
      setValue('designitionable_id', campuses[0].id ?? 0);
    }
  }, [campuses, setValue, watch, isEdit]);
  ...
  const onFormSubmit = useCallback(
    async (data: CampusRegistrarFormData) => {
      try {
        if (isEdit) {
        ...
        toast.success(`Campus Registrar ${isEdit ? 'updated' : 'created'} successfully`);
        controller.closeFn();
        onSubmit(data);
      } catch (error) {
        renderError(error, setError);
        toast.error(`Failed to ${isEdit ? 'update' : 'create'} Campus Registrar`);
      }
    },
    [createCampusRegistrar, updateDesignition, controller, onSubmit, setError, isEdit]
  );
  ...
  return (
    <Modal
      ...
      title={`${isEdit ? 'Edit' : 'Add'} Campus Registrar`}
      size="half"
      closable
    >
...
export default function CampusRegistrarModal(props: CampusRegistrarModalProps) {
  return (
    <DndProvider backend={HTML5Backend}>
      <CampusRegistrarModalContent {...props} />
    </DndProvider>
  );
}
