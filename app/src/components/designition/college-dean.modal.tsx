...
const collegeDeanSchema = z.object({
  user_id: z.number().min(1, 'User is required'),
  designitionable_id: z.number().min(1, 'College is required'),
  is_active: z.boolean(),
});

type CollegeDeanFormData = z.infer<typeof collegeDeanSchema>;

interface CollegeDeanModalProps {
  controller: ModalState<Designition>;
  onSubmit: (data: CollegeDeanFormData) => void;
}
...
function CollegeDeanModalContent({ controller, onSubmit }: CollegeDeanModalProps) {
  const {
    handleSubmit,
    reset,
    setError,
    ...
    setValue,
    formState: { errors },
  } = useForm<CollegeDeanFormData>({
    resolver: zodResolver(collegeDeanSchema),
    defaultValues: {
      user_id: 0,
  ...
  const { mutateAsync: createCollegeDean, isPending } = useCreateCollegeDean();
  ...
  useEffect(() => {
    if (colleges.length > 0 && watch('designitionable_id') === 0 && !isEdit) {
      setValue('designitionable_id', colleges[0].id ?? 0);
    }
  }, [colleges, setValue, watch, isEdit]);
  ...
  const onFormSubmit = useCallback(
    async (data: CollegeDeanFormData) => {
      try {
        if (isEdit) {
        ...
        toast.success(`College Dean ${isEdit ? 'updated' : 'created'} successfully`);
        controller.closeFn();
        onSubmit(data);
      } catch (error) {
        renderError(error, setError);
        toast.error(`Failed to ${isEdit ? 'update' : 'create'} College Dean`);
      }
    },
    [createCollegeDean, updateDesignition, controller, onSubmit, setError, isEdit]
  );
  ...
  const handleCampusChange = useCallback(
    (value: string) => {
      setFilters((prev) => ({ ...prev, campus_id: Number(value) }));
  ...
  return (
    <Modal
      ...
      title={`${isEdit ? 'Edit' : 'Add'} College Dean`}
      size="half"
      closable
    >
...
export default function CollegeDeanModal(props: CollegeDeanModalProps) {
  return (
    <DndProvider backend={HTML5Backend}>
      <CollegeDeanModalContent {...props} />
    </DndProvider>
  );
}
