...
import { useCreateAdmissionOfficer, useGetUserPaginated, useUpdateDesignition } from '@rest/api';
...
const admissionOfficerSchema = z.object({
  user_id: z.number().min(1, 'User is required'),
  is_active: z.boolean(),
});

type AdmissionOfficerFormData = z.infer<typeof admissionOfficerSchema>;

interface AdmissionOfficerModalProps {
  controller: ModalState<Designition>;
  onSubmit: (data: AdmissionOfficerFormData) => void;
}
...
function AdmissionOfficerModalContent({ controller, onSubmit }: AdmissionOfficerModalProps) {
  const {
    handleSubmit,
    reset,
    setError,
    ...
    setValue,
    formState: { errors },
  } = useForm<AdmissionOfficerFormData>({
    resolver: zodResolver(admissionOfficerSchema),
    defaultValues: {
      user_id: 0,
      is_active: true,
    },
  ...
  const { mutateAsync: createAdmissionOfficer, isPending } = useCreateAdmissionOfficer();
  ...
  const onFormSubmit = useCallback(
    async (data: AdmissionOfficerFormData) => {
      try {
        if (isEdit) {
        ...
        toast.success(`Admission Officer ${isEdit ? 'updated' : 'created'} successfully`);
        controller.closeFn();
        onSubmit(data);
      } catch (error) {
        renderError(error, setError);
        toast.error(`Failed to ${isEdit ? 'update' : 'create'} Admission Officer`);
      }
    },
    [createAdmissionOfficer, updateDesignition, controller, onSubmit, setError, isEdit]
  );
  ...
  return (
    <Modal
      ...
      title={`${isEdit ? 'Edit' : 'Add'} Admission Officer`}
      size="half"
      closable
    >
...
export default function AdmissionOfficerModal(props: AdmissionOfficerModalProps) {
  return (
    <DndProvider backend={HTML5Backend}>
      <AdmissionOfficerModalContent {...props} />
    </DndProvider>
  );
}
