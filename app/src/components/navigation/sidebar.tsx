...
export default function SideBar({
  ...
  const computedRoutes = useMemo(() => {
    const routes = ROUTES.filter(
      (route) => route.sidebar === true || route.sidebar === undefined
    ).filter((route) =>
      (route.roles ?? []).some((role) => user?.session?.roles?.some((r) => r === role))
    );

    // Simulate loading - in real scenario, this would be based on actual data loading state
  ...
  return (
    ...
    >
      ...
      >
        {isLoading ? (
          <div className={cn('flex items-center gap-2 flex-1', isCollapsed && 'lg:hidden')}>
      ...
      >
        {isLoading ? (
          // Skeleton loaders for navigation items
          <>
            {[...Array(6)].map((_, index) => (
              <div key={index} className="space-y-1">
                ...
                >
                  ...
                  />
                  <Skeleton
                    className={cn('h-4 flex-1', isCollapsed && 'lg:max-w-0 lg:opacity-0')}
                  />
                </div>
              </div>
            ))}
                ...
                <div className="relative">
                  <Link
                    ...
                    className={cn(
                      ...
                      isCollapsed
                        ? 'lg:px-1.5 lg:py-2 lg:justify-center px-2.5 py-2'
                        : 'px-2.5 py-2',
                      isActive
