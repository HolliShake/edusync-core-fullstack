...
import type { AdmissionApplication, UniversityAdmissionApplication } from '@rest/models';
import {
  ArrowLeft,
  ArrowRight,
  Check,
...
export default function GuestAdmissionSelectProgramStep4({
  ...
  const { data: admissionCriteriaResponse, isLoading: isLoadingRequirements } =
    useGetAdmissionCriteriaPaginated(
      {
        'filter[admission_schedule_id]': fields.admissionScheduleId,
      },
  ...
  const isStep1Complete =
    fields.campusId && fields.schoolYearId && fields.collegeId && fields.admissionScheduleId;
      ...
      <div className="flex-1 flex flex-col bg-muted/10 h-full overflow-y-auto">
        ...
        <div className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-12 lg:p-16 flex flex-col justify-center min-h-[calc(100vh-4rem)]">
          <form
            ...
            className="space-y-8 animate-in fade-in duration-700 slide-in-from-bottom-4"
          >
            <div className="mb-8">
              ...
              <p className="text-muted-foreground mt-2 text-lg">
                {steps[currentStep - 1].number === 1 &&
                  'Select your academic preferences and upload requirements.'}
                {steps[currentStep - 1].number === 2 &&
                  'Provide your personal details for your student record.'}
                {steps[currentStep - 1].number === 3 &&
                  'Help us stay in touch with you regarding your application.'}
              </p>
            </div>
                        ...
                        {!isLoadingRequirements && academicProgramRequirements.length > 0 && (
                          <div className="hidden sm:flex items-center gap-2 bg-secondary/50 px-3 py-1.5 rounded-full">
                            ...
                            <span
                              className={cn(
                                'text-xs font-bold',
                                mandatoryRequirements.filter((req) => uploadedFiles[req.id!])
                                  .length === mandatoryRequirements.length
                                  ? 'text-green-600 dark:text-green-500'
                                  : 'text-primary'
                              )}
                            >
                              {mandatoryRequirements.filter((req) => uploadedFiles[req.id!]).length}{' '}
                      ...
                      {isLoadingRequirements ? (
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2 mb-0.5">
                                    <h4
                                      className={cn(
                                        ...
                                        isUploaded && 'text-primary'
                                      )}
                                    >
                                      {req.requirement?.requirement_name}
                                    </h4>
                                    {req.requirement?.is_mandatory && !isUploaded && (
                                  ...
                                  {isUploaded ? (
                                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                      ...
                                      <span>
                                        {(uploadedFiles[req.id!].size / 1024).toFixed(0)} KB
                                      </span>
                                    </div>
                                  ) : (
                                  ...
                                  {isUploaded ? (
                                    ...
                                    <div className="relative">
                                      ...
                                      <Button
                                        type="button"
                                        variant={
                                          req.requirement?.is_mandatory ? 'default' : 'outline'
                                        }
                                        size="sm"
                                        asChild
                                        className={cn(
                        ...
                        <div className="text-center py-12 px-4 text-muted-foreground bg-muted/20 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800">
                          ...
                          <p className="text-sm mt-1 max-w-xs mx-auto">
                            There are no specific documents required for this program at this time.
                          </p>
                        </div>
                      )}
                    </div>
              ...
              {currentStep === 2 && (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-3">
                      <Label className="font-medium" htmlFor="middle_name">
                        Middle Name
                      </Label>
                      <Input
                        id="middle_name"
